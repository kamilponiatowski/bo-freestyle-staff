...
// Odśwież wykresy przy zmianie danych
...
const updateProgressChart = () => {
  if (!progressChartInstance.value) return;
  
  const chartData = prepareProgressChartData();
  
  progressChartInstance.value.data.labels = chartData.labels;
  progressChartInstance.value.data.datasets = chartData.datasets;
  progressChartInstance.value.update();
};

// Aktualizacja wykresu kategorii
const updateCategoryChart = () => {
  if (!categoryChartInstance.value) return;
  
  const chartData = prepareCategoryChartData();
  
  categoryChartInstance.value.data.labels = chartData.labels;
  categoryChartInstance.value.data.datasets = chartData.datasets;
...
const addTraining = () => {
  ...
  // Dodaj powtórzenia
  const success = DataService.addReps(trainingSkill.value, parseInt(trainingReps.value));
  ...
  if (success) {
    ...
    if (skill && skill.status === 'completed') {
      // Tutaj można dodać kod wyświetlający gratulacje
    }
  } else {
